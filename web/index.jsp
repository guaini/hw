...
<div class="description">
  <h1>欢迎来到我的画廊</h1>
  <p>这是一个工程模拟训练期末作业，前端使用 Vue + Axios，后端使用 mysql</p>
...
<script>
  function log_out() {
    ...
    <%--  action: "logout",--%>
    <%--}, {--%>
    <%--  headers: {Authorization: ` "<%=session.getAttribute("username")%>"`},--%>
    <%--})--%>
  ...
  function start(choice) {
    ...
    rowTotalWidth = 0;
    if (choice === 'date') {
      axios.get("http://43.139.215.45:80/api/Index?action=date").then(render);
    } else if (choice === 'pid') {
      axios.get("http://43.139.215.45:80/api/Index?action=pid").then(render);
    } else if (choice === 'star') {
      axios.get("http://43.139.215.45:80/api/Index?action=star").then(render);
    } else if (choice === 'like') {
      axios.get("http://43.139.215.45:80/api/Index?action=like").then(render);
    } else if (choice === 'pname') {
      axios.get("http://43.139.215.45:80/api/Index?action=name").then(render);
    } else if (choice === '') {
      getData().then(render)
    } else {
      axios.get("http://43.139.215.45:80/search?key=" + choice).then(render);
    }
  }
  ...
  function getData() {
    ...
    var url = "http://43.139.215.45:80/api/Picture";
  ...
  function render(response) {
    ...
    response.data.picList.forEach(function (imgInfo) {
      ...
      imgInfo.src = "http://43.139.215.45:80" + imgInfo.url;
